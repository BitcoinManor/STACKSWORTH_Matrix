...
bool fetchDaysSinceAthFromSatoNak() {
  ...
  if (request->hasParam("ssid", true) && request->hasParam("password", true)) {
    ...
    void loop()
    {
      ...
      // 🚀 INITIAL FETCH - Run once on first loop iteration (safe for lwIP)
      // ⚠️ CONSERVATIVE: Only try SatoNak APIs, skip fallbacks to prevent WDT crashes
      // If SatoNak is down, we use cached values and retry in normal loop cycle
      if (wifiConnected && !initialFetchDone && !apMode) {
        Serial.println("🌍 Fetching initial data (first loop iteration - SatoNak only, no fallbacks)...");
        Serial.println("📊 Detailed API logging enabled to track crash culprits...");
        
        unsigned long now = millis();
        
        // Try each API but don't hang if they fail - cached data will be used
        Serial.println("🔍 [1/11] Fetching Block Height..."); esp_task_wdt_reset(); fetchHeightFromSatoNak(); lastBlockHeightFetch = now;
        Serial.println("✅ [1/11] Block Height fetch complete");
        delay(500);
        Serial.println("🔍 [2/11] Fetching Miner..."); esp_task_wdt_reset(); fetchMinerFromSatoNak(); lastMinerFetch = now;
        Serial.println("✅ [2/11] Miner fetch complete");
        delay(500);
        Serial.println("🔍 [3/11] Fetching Price..."); esp_task_wdt_reset(); fetchPriceFromSatoNak(); lastPriceFetch = now;  // Direct SatoNak only, no CoinGecko fallback
        Serial.println("✅ [3/11] Price fetch complete");
        delay(500);
        Serial.println("🔍 [4/11] Fetching 24H Change..."); esp_task_wdt_reset(); fetchChange24hFromSatoNak(); lastChange24hFetch = now;
        Serial.println("✅ [4/11] 24H Change fetch complete");
        delay(500);
        Serial.println("🔍 [5/11] Fetching Fee Rate..."); esp_task_wdt_reset(); fetchFeeFromSatoNak(); lastFeeFetch = now;  // Direct SatoNak only, no mempool.space fallback
        Serial.println("✅ [5/11] Fee Rate fetch complete");
        delay(500);
        Serial.println("🔍 [6/11] Fetching Hashrate..."); esp_task_wdt_reset(); fetchHashrateFromSatoNak(); lastHashrateFetch = now;
        Serial.println("✅ [6/11] Hashrate fetch complete");
        delay(500);
        Serial.println("🔍 [7/11] Fetching Circulating Supply..."); esp_task_wdt_reset(); fetchCircSupplyFromSatoNak(); lastCircSupplyFetch = now;
        Serial.println("✅ [7/11] Circulating Supply fetch complete");
        delay(500);
        Serial.println("🔍 [8/11] Fetching ATH..."); esp_task_wdt_reset(); fetchAthFromSatoNak(); lastAthFetch = now;
        Serial.println("✅ [8/11] ATH fetch complete");
        delay(500);
        Serial.println("🔍 [9/11] Fetching Days Since ATH..."); esp_task_wdt_reset(); fetchDaysSinceAthFromSatoNak(); lastDaysAthFetch = now;
        Serial.println("✅ [9/11] Days Since ATH fetch complete");
        delay(500);
        Serial.println("🔍 [10/11] Fetching Time..."); esp_task_wdt_reset(); fetchTime();
        Serial.println("✅ [10/11] Time fetch complete");
        delay(100);
        Serial.println("🔍 [11/11] Fetching Location & Weather..."); esp_task_wdt_reset(); fetchLatLonFromCity();
        delay(100);
        esp_task_wdt_reset(); fetchWeather();
        Serial.println("✅ [11/11] Location & Weather fetch complete");

        lastFetchTime = millis();
        Serial.println("✅ Initial data fetch complete! (Fallback APIs will retry in normal loop if needed)");
        saveDisplayCache();
